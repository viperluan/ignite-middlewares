...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  const proAccount = user.pro;
  const freeAccountPass = user.todos.length < 10;

  if (!proAccount && !freeAccountPass) {
    return response
      .status(403)
      .json({ error: "You don't have permission to have more todos!" });
  }
